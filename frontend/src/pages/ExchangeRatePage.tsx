...
import {
    TrendingUp, Plus, Trash2, Edit, RefreshCw, Search, AlertCircle,
    Filter, SortAsc, SortDesc, Calendar, ArrowRightLeft, DollarSign,
    Eye, MoreVertical, Clock, Activity
} from 'lucide-react';
import {
...
const ExchangeRatePage = ({ showModal }) => {
    ...
    const [sortConfig, setSortConfig] = useState({ key: 'fromCurrency', direction: 'asc' });
    ...
    const {
        ...
        onError: (err) => showModal(err.message || 'Failed to load exchange rates.', 'Error'),
    });
    ...
    const createExchangeRateMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to create exchange rate.', 'Error'),
    });
    ...
    const updateExchangeRateMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to update exchange rate.', 'Error'),
    });
    ...
    const deleteExchangeRateMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to delete exchange rate.', 'Error'),
    });
    ...
    const filteredAndSortedRates = React.useMemo(() => {
        ...
        if (dateFilter) {
            filtered = filtered.filter(rate =>
                new Date(rate.effectiveDate).toDateString() === new Date(dateFilter).toDateString()
            );
        }
    ...
    // Permission checks
    const canManageRates = currentUser?.permissions?.exchangeRates?.includes('manage') ||
    ...
    const stats = React.useMemo(() => {
        ...
        const recentRates = exchangeRates.filter(rate =>
            new Date(rate.effectiveDate).toDateString() === today
        );
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
                <div className="text-center bg-white p-8 rounded-xl shadow-lg border border-red-200">
                    ...
                    {ratesError && <p className="text-red-600 mb-2">{ratesError.message}</p>}
                    {currenciesError && <p className="text-red-600">{currenciesError.message}</p>}
                    <button
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
            ...
            <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-gray-600 text-sm font-medium">Today's Rates</p>
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-purple-600">{stats.currencies}</p>
                            </div>
                    ...
                    {filteredAndSortedRates.length > 0 ? (
                            ...
                            <table className="w-full">
                                ...
                                {filteredAndSortedRates.map(rate => (
                                    <tr key={rate.id} className="hover:bg-gray-50 transition-colors duration-150">
                                        <td className="py-4 px-6 font-mono font-semibold text-gray-900">{rate.fromCurrency}</td>
                                        <td className="py-4 px-6 font-mono font-semibold text-gray-900">{rate.toCurrency}</td>
                                        <td className="py-4 px-6 text-gray-900 font-medium">{rate.rate.toFixed(6)}</td>
                        ...
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                {searchTerm || dateFilter || currencyFilter ? 'No exchange rates found' : 'No exchange rates yet'}
                            </h3>
                            ...
                            {canManageRates && !(searchTerm || dateFilter || currencyFilter) && (
                                <button
                                    onClick={handleCreateRate}
            ...
            {isModalOpen && (
                <ExchangeRateFormModal
                    ...
                    isLoading={createExchangeRateMutation.isLoading || updateExchangeRateMutation.isLoading}
                />
            )}
        </div>
    );
