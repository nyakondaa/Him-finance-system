...
import {
    DollarSign, CreditCard, Plus, Trash2, Edit, ChevronDown, ChevronUp,
    Search, RefreshCw, List, X, Check, AlertCircle, Settings, Eye,
    Filter, SortAsc, SortDesc, Grid, Table, MoreVertical, Zap, Star
} from 'lucide-react';
import {
    getCurrencies, createCurrency, updateCurrency, deleteCurrency,
    getPaymentMethods, getCurrencyPaymentMethods, updateCurrencyPaymentMethod,
    deleteCurrencyPaymentMethod, getPaymentMethodCurrencies, updatePaymentMethodCurrency,
    deletePaymentMethodCurrency, createPaymentMethod, updatePaymentMethod, deletePaymentMethod
} from '../services/api';
...
const CombinedCurrencyPaymentMethodPage = ({ showModal }) => {
    ...
    const {
        ...
        onError: (err) => showModal(err.message || 'Failed to load currencies.', 'Error'),
    });
    ...
    const {
        ...
        onError: (err) => showModal(err.message || 'Failed to load payment methods.', 'Error'),
    });
    ...
    const createCurrencyMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to create currency.', 'Error'),
    });
    ...
    const updateCurrencyMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to update currency.', 'Error'),
    });
    ...
    const deleteCurrencyMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to delete currency.', 'Error'),
    });
    ...
    const createPaymentMethodMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to create payment method.', 'Error'),
    });
    ...
    const updatePaymentMethodMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to update payment method.', 'Error'),
    });
    ...
    const deletePaymentMethodMutation = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to delete payment method.', 'Error'),
    });
    ...
    const updatePaymentMethodForCurrency = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to update payment method association.', 'Error'),
    });
    ...
    const updateCurrencyForPaymentMethod = useMutation({
        ...
        onError: (err) => showModal(err.message || 'Failed to update currency association.', 'Error'),
    });
    ...
    const handleDeleteItem = (id) => {
        showModal(
            `Are you sure you want to delete this ${activeTab === 'currencies' ? 'currency' : 'payment method'}? This action cannot be undone.`,
            'Confirm Deletion',
            true,
            () => {
                if (activeTab === 'currencies') {
    ...
    const filteredAndSortedItems = React.useMemo(() => {
        const items = activeTab === 'currencies' ? [...currencies] : [...paymentMethods];
        ...
        filtered = filtered.filter(item => {
            const searchFields = activeTab === 'currencies'
        ...
        return filtered;
    }, [activeTab === 'currencies' ? currencies : paymentMethods, sortConfig, searchTerm, statusFilter]);

    // Fixed permission checks - using more defensive approach
    const canManageCurrencies = currentUser?.permissions?.currencies?.includes('manage') ||
        currentUser?.role === 'admin' ||
        currentUser?.role === 'super_admin' ||
        false;

    const canViewCurrencies = currentUser?.permissions?.currencies?.includes('read') ||
        canManageCurrencies ||
        false;

    const canManagePaymentMethods = currentUser?.permissions?.paymentMethods?.includes('manage') ||
        currentUser?.permissions?.payment_methods?.includes('manage') || // alternative naming
        currentUser?.role === 'admin' ||
        currentUser?.role === 'super_admin' ||
        false;

    const canViewPaymentMethods = currentUser?.permissions?.paymentMethods?.includes('read') ||
    ...
    const stats = React.useMemo(() => {
        const currentData = activeTab === 'currencies' ? currencies : paymentMethods;
        return {
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
                <div className="text-center bg-white p-8 rounded-xl shadow-lg border border-red-200">
                    ...
                    {currenciesError && <p className="text-red-600 mb-2">{currenciesError.message}</p>}
                    {paymentMethodsError && <p className="text-red-600">{paymentMethodsError.message}</p>}
                    <button
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div>
                        <h1 className="text-4xl font-bold text-gray-900 flex items-center gap-3 mb-2">
                            ...
                            {activeTab === 'currencies' ? 'Currency Management' : 'Payment Methods'}
                        </h1>
                        <p className="text-gray-600">
                            Manage your {activeTab === 'currencies' ? 'currencies and their relationships' : 'payment methods and configurations'}
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex items-center justify-between">
                            ...
                            <div className={`p-3 rounded-lg ${activeTab === 'currencies' ? 'bg-green-100' : 'bg-blue-100'}`}>
                                <List className={`w-6 h-6 ${activeTab === 'currencies' ? 'text-green-600' : 'text-blue-600'}`} />
                            </div>
                        </div>
                    </div>
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <div className="p-6 border-b border-gray-200 bg-gray-50">
                    <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                        ...
                        <div className="flex items-center gap-2 w-full lg:w-auto">
                            ...
                            {/* Refresh Button */}
                            <button
                                onClick={activeTab === 'currencies' ? refetchCurrencies : refetchPaymentMethods}
                            ...
                            {/* Create Button */}
                            {(activeTab === 'currencies' ? canManageCurrencies : canManagePaymentMethods) && (
                                <button
                                    ...
                                    className={`px-6 py-3 text-white rounded-xl hover:opacity-90 transition duration-300 shadow-lg flex items-center gap-2 font-medium ${
                                        activeTab === 'currencies' ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'
                                    }`}
                                >
                ...
                <div className="p-6">
                    {viewMode === 'table' ? (
                        <TableView
                            ...
                            canManage={activeTab === 'currencies' ? canManageCurrencies : canManagePaymentMethods}
            ...
            {isCurrencyModalOpen && (
                <CurrencyFormModal
                    ...
                    isLoading={createCurrencyMutation.isLoading || updateCurrencyMutation.isLoading}
                />
            )}
            ...
            {isPaymentMethodModalOpen && (
                <PaymentMethodFormModal
                    ...
                    isLoading={createPaymentMethodMutation.isLoading || updatePaymentMethodMutation.isLoading}
                />
            )}
        </div>
    );
...
const TableView = ({
                       items, activeTab, sortConfig, requestSort, expandedItem, setExpandedItem,
                       handleEditItem, handleDeleteItem, canManage, currencies, paymentMethods,
                       updatePaymentMethodForCurrency, updateCurrencyForPaymentMethod,
                       deleteCurrencyPaymentMethod, deletePaymentMethodCurrency, queryClient, showModal
                   }) => (
    <div className="overflow-x-auto">
        <table className="w-full">
            ...
            <tr className="border-b border-gray-200">
                {activeTab === 'currencies' ? (
                    <>
                        <SortableHeader title="Code" sortKey="code" sortConfig={sortConfig} requestSort={requestSort} />
                ...
                <SortableHeader title="Status" sortKey="isActive" sortConfig={sortConfig} requestSort={requestSort} />
            ...
            {items.map(item => {
                const isExpanded = expandedItem === (activeTab === 'currencies' ? item.code : item.id);
                return (
                    <React.Fragment key={activeTab === 'currencies' ? item.code : item.id}>
                        <tr className="hover:bg-gray-50 transition-colors duration-150">
                            {activeTab === 'currencies' ? (
                                <>
                                    <td className="py-4 px-6 font-mono font-semibold text-gray-900">{item.code}</td>
                                    <td className="py-4 px-6 text-gray-900">{item.name}</td>
                                    <td className="py-4 px-6 text-gray-600 font-mono">{item.symbol || '-'}</td>
                                </>
                            ) : (
                                <>
                            ...
                            <td className="py-4 px-6">
                                <button
                                    onClick={() => setExpandedItem(isExpanded ? null : (activeTab === 'currencies' ? item.code : item.id))}
                                ...
                                <td colSpan={canManage ? 6 : 5} className="px-6 py-6">
                                    {activeTab === 'currencies' ? (
                                        <ExpandedCurrencyView
                                            ...
                                            paymentMethods={paymentMethods}
                                            onTogglePaymentMethod={(currencyCode, paymentMethodId, isActive) =>
                                                updatePaymentMethodForCurrency.mutate({ currencyCode, paymentMethodId, isActive })
                                            }
                                            onRemovePaymentMethod={(currencyCode, paymentMethodId) =>
                                                deleteCurrencyPaymentMethod(currencyCode, paymentMethodId)
                                                    .then(() => {
                                                        queryClient.invalidateQueries(['currencies']);
                                                        showModal('Payment method removed successfully.', 'Success');
                                                    })
                                                    .catch(err => showModal(err.message || 'Failed to remove payment method.', 'Error'))
                                            }
                                            onAddPaymentMethod={(currencyCode, paymentMethodId) =>
                                                updatePaymentMethodForCurrency.mutate({ currencyCode, paymentMethodId, isActive: true })
                                            }
                                        />
                                    ) : (
                                        <ExpandedPaymentMethodView
                                            ...
                                            currencies={currencies}
                                            onToggleCurrency={(paymentMethodId, currencyCode, isActive) =>
                                                updateCurrencyForPaymentMethod.mutate({ paymentMethodId, currencyCode, isActive })
                                            }
                                            onRemoveCurrency={(paymentMethodId, currencyCode) =>
                                                deletePaymentMethodCurrency(paymentMethodId, currencyCode)
                                                    .then(() => {
                                                        queryClient.invalidateQueries(['paymentMethods']);
                                                        showModal('Currency removed successfully.', 'Success');
                                                    })
                                                    .catch(err => showModal(err.message || 'Failed to remove currency.', 'Error'))
                                            }
                                            onAddCurrency={(paymentMethodId, currencyCode) =>
                                                updateCurrencyForPaymentMethod.mutate({ paymentMethodId, currencyCode, isActive: true })
                                            }
                                        />
                                    )}
                                </td>
...
const GridView = ({ items, activeTab, handleEditItem, handleDeleteItem, canManage }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {items.map(item => (
            <div key={activeTab === 'currencies' ? item.code : item.id}
                ...
                <div className="flex justify-between items-start mb-4">
                    <div className={`p-3 rounded-xl ${activeTab === 'currencies' ? 'bg-green-100' : 'bg-blue-100'}`}>
                        {activeTab === 'currencies' ? (
                            <DollarSign className={`w-6 h-6 ${activeTab === 'currencies' ? 'text-green-600' : 'text-blue-600'}`} />
                        ) : (
                            <CreditCard className={`w-6 h-6 ${activeTab === 'currencies' ? 'text-green-600' : 'text-blue-600'}`} />
                        )}
                    </div>
                    <StatusBadge isActive={item.isActive} />
                </div>
                ...
                <div className="mb-4" onClick={() => canManage && handleEditItem(item)}>
                    {activeTab === 'currencies' ? (
                        <>
                            <h3 className="font-bold text-lg text-gray-900 mb-1">{item.code}</h3>
                            <p className="text-gray-600 mb-2">{item.name}</p>
                            {item.symbol && (
                                <p className="text-sm text-gray-500 font-mono">Symbol: {item.symbol}</p>
                            )}
                        </>
                    ) : (
                        <>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                    ...
                    {canManage && (
                        <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                            ...
                            <button
                                onClick={() => handleDeleteItem(activeTab === 'currencies' ? item.code : item.id)}
...
const EmptyState = ({ activeTab, searchTerm, handleCreateItem, canManage }) => (
    <div className="text-center py-12">
        <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${
            ...
            {activeTab === 'currencies' ? (
                <DollarSign className={`w-8 h-8 ${activeTab === 'currencies' ? 'text-green-600' : 'text-blue-600'}`} />
            ) : (
                <CreditCard className={`w-8 h-8 ${activeTab === 'currencies' ? 'text-green-600' : 'text-blue-600'}`} />
            )}
        </div>
        ...
        <p className="text-gray-600 mb-6">
            {searchTerm
                ...
                : `Get started by creating your first ${activeTab === 'currencies' ? 'currency' : 'payment method'}.`
            }
        </p>
...
const ExpandedCurrencyView = ({
    ...
    const filteredAvailableMethods = paymentMethods.filter(pm => {
        ...
        return pm.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                ...
                {currencyPaymentMethods.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {currencyPaymentMethods.map(pm => (
                            <div key={pm.id} className="bg-white border-2 border-gray-100 rounded-xl p-4 hover:border-gray-200 transition-colors">
                                ...
                                <div className="flex items-center justify-between">
                                    <label className="flex items-center gap-3 cursor-pointer">
                                        <input
                                            type="checkbox"
                                            checked={pm.isActive}
                                            onChange={(e) => onTogglePaymentMethod(currencyCode, pm.id, e.target.checked)}
                                            className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                        />
                                        <span className={`text-sm font-medium ${pm.isActive ? 'text-green-700' : 'text-gray-500'}`}>
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-gray-600">No payment methods configured for this currency</p>
                    </div>
                )}
            </div>
                ...
                {filteredAvailableMethods.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredAvailableMethods.map(pm => (
                            <div key={pm.id} className="bg-gray-50 border border-gray-200 rounded-xl p-4 hover:bg-gray-100 transition-colors">
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-gray-600">
                            {searchTerm ? 'No matching payment methods found' : 'All available payment methods are already added'}
                        </p>
                    </div>
                )}
            </div>
                ...
                {paymentMethodCurrencies.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {paymentMethodCurrencies.map(currency => (
                            <div key={currency.code} className="bg-white border-2 border-gray-100 rounded-xl p-4 hover:border-gray-200 transition-colors">
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <h5 className="font-semibold text-gray-900 font-mono">{currency.code}</h5>
                                        <p className="text-sm text-gray-600">{currency.name}</p>
                                        {currency.symbol && (
                                            <p className="text-xs text-gray-500 font-mono mt-1">Symbol: {currency.symbol}</p>
                                        )}
                                    </div>
                                    <button
                                ...
                                <div className="flex items-center justify-between">
                                    <label className="flex items-center gap-3 cursor-pointer">
                                        <input
                                            ...
                                            onChange={(e) => onToggleCurrency(paymentMethodId, currency.code, e.target.checked)}
                                            className="h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
                                        />
                                        <span className={`text-sm font-medium ${currency.isActive ? 'text-green-700' : 'text-gray-500'}`}>
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-gray-600">No currencies configured for this payment method</p>
                    </div>
                )}
            </div>
                ...
                {filteredAvailableCurrencies.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredAvailableCurrencies.map(currency => (
                            <div key={currency.code} className="bg-gray-50 border border-gray-200 rounded-xl p-4 hover:bg-gray-100 transition-colors">
                                <div className="flex justify-between items-center">
                                    <div className="flex-grow">
                                        <h5 className="font-semibold text-gray-900 font-mono">{currency.code}</h5>
                                        <p className="text-sm text-gray-600">{currency.name}</p>
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                        <Search className="w-12 h-12 text-gray-400 mx-auto mb-3" />
                        <p className="text-gray-600">
                            {searchTerm ? 'No matching currencies found' : 'All available currencies are already added'}
                        </p>
                    </div>
                )}
            </div>
