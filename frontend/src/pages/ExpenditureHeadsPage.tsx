...
const ExpenditureHeadsPage = ({ showModal }) => {
  ...
  const {
    data: expenditureHeads = [],
    isLoading: isLoadingExpenditureHeads,
    isError: isErrorExpenditureHeads,
    error: expenditureHeadsError,
  } = useQuery({
    queryKey: ["expenditureHeads"],
    queryFn: getExpenditureHeads,
    onError: (err) =>
      showModal(err.message || "Failed to load expenditure heads.", "Error"),
  });
  ...
  const handleAddExpenditure = () => {
    if (!newExpenditure.name || !newExpenditure.branch) {
      showModal(
        "Please fill in both name and branch for the new expenditure head.",
        "Validation Error"
      );
      return;
    }
  ...
  const handleUpdateExpenditure = () => {
    if (!editingExpenditure.name || !editingExpenditure.branchCode) {
      showModal(
        "Please fill in both name and branch for the expenditure head.",
        "Validation Error"
      );
      return;
    }
    updateExpenditureMutation.mutate({
      code: editingExpenditure.code,
      data: {
        name: editingExpenditure.name,
        branch: editingExpenditure.branchCode,
      },
    });
  };
    ...
    return (
      <div className="bg-white p-8 rounded-xl shadow-lg text-center text-gray-700">
        <p className="text-lg font-semibold">
          You do not have permission to access Expenditure Heads Setup.
        </p>
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg mb-8 border border-gray-200">
        <h3 className="text-xl font-semibold mb-6 text-gray-700">
          {editingExpenditure
            ? "Edit Expenditure Head"
            : "Add New Expenditure Head"}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <input
            ...
            value={
              editingExpenditure ? editingExpenditure.name : newExpenditure.name
            }
            onChange={(e) =>
              editingExpenditure
          ...
          <select
            value={
              editingExpenditure
                ? editingExpenditure.branchCode
                : newExpenditure.branch
            }
            onChange={(e) =>
              editingExpenditure
                ? setEditingExpenditure({
        ...
        <div className="flex gap-4 mt-6">
          <button
            ...
            disabled={
              (editingExpenditure
                ? !editingExpenditure.name
                : !newExpenditure.name) ||
              addExpenditureMutation.isLoading ||
              updateExpenditureMutation.isLoading
            }
          >
            {addExpenditureMutation.isLoading ||
            updateExpenditureMutation.isLoading ? (
              ...
              <>
                <Save className="w-5 h-5" /> Update Expenditure Head
              </>
            ) : (
              <>
                <Plus className="w-5 h-5" /> Add Expenditure Head
              </>
            )}
          </button>
          {editingExpenditure && (
              ...
              {expenditureHeads.map((expenditure) => (
                ...
                >
                  ...
                  <td className="py-3 px-6 text-left">
                    {expenditure.branchCode} -{" "}
                    {
                      branches.find((b) => b.code === expenditure.branchCode)
                        ?.name
                    }
                  </td>
                  <td className="py-3 px-6 text-left flex items-center gap-2">
                    ...
                    >
                      {deleteExpenditureMutation.isLoading ? (
                        "Deleting..."
                      ) : (
                        <>
