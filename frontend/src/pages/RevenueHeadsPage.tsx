...
const RevenueHeadsPage = ({ showModal }) => {
  ...
  const handleAddRevenue = () => {
    if (!newRevenue.name || !newRevenue.branchCode) {
      showModal(
        "Please fill in both name and branch for the new revenue head.",
        "Validation Error"
      );
      return;
    }
  ...
  const handleUpdateRevenue = () => {
    if (!editingRevenue?.name || !editingRevenue?.branchCode) {
      showModal(
        "Please fill in both name and branch for the revenue head.",
        "Validation Error"
      );
      return;
    }
    ...
    return (
      <div className="bg-white p-8 rounded-xl shadow-lg text-center text-gray-700">
        <p className="text-lg font-semibold">
          You do not have permission to access Revenue Heads Setup.
        </p>
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg mb-8 border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <select
            value={
              editingRevenue ? editingRevenue.branchCode : newRevenue.branchCode
            }
            onChange={(e) =>
              editingRevenue
                ? setEditingRevenue({
        ...
        <div className="flex gap-4 mt-6">
          <button
            ...
            disabled={
              (editingRevenue
                ? !editingRevenue.name || !editingRevenue.branchCode
              ...
              {revenueHeads.map((revenue) => (
                ...
                >
                  ...
                  <td className="py-3 px-6 text-left">
                    {revenue.branchCode} -{" "}
                    {branches.find((b) => b.code === revenue.branchCode)
                      ?.name || "Unknown"}
                  </td>
                  <td className="py-3 px-6 text-left flex items-center gap-2">
                    ...
                    <button
                      onClick={() =>
                        handleDeleteRevenue(revenue.code, revenue.name)
                      }
