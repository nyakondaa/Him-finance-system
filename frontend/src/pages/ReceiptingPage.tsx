...
const ReceiptingPage = () => {
  ...
  /* {this is the old connect to QZ function}
    ...
    try {
      ...
      window.qz.security.setCertificatePromise(() => {
        ...
        return fetch(`${API_BASE_URL}/qz/certificate`).then((response) => {
          console.log("Fetched certificate from server");
...
{old qz function from privous edi}
*/ 
...
const connectToQz = useCallback(async () => {
  ...
  try {
    ...
    if (typeof window.qz === 'undefined') {
      throw new Error("QZ Tray is not available. Please install and launch QZ Tray.");
    }

    // Set up security promises - use simple mode for development
    window.qz.security.setCertificatePromise(() => {
      return Promise.resolve(""); // Empty certificate for development
    });
    ...
    if (window.qz.websocket.isActive()) {
      ...
      showModal(
        `QZ Tray already connected! Found ${printers.length} printer(s).`,
        "Connection Successful",
        "success"
      );
      return;
    ...
    let errorMessage = "Could not connect to QZ Tray. ";

    if (err.message.includes("not available")) {
      errorMessage += "Please install and launch QZ Tray from qz.io.";
    } else if (err.message.includes("already exists") || err.message.includes("already connected")) {
      errorMessage = "QZ Tray connection already established";
      // Try to get printers anyway
      try {
        const printers = await window.qz.printers.find();
      ...
      errorMessage += "Please ensure QZ Tray is running and try again.";
    }
  ...
  const getStatusInfo = () => {
    switch (status) {
      case "connected":
        return { text: "Connected to QZ Tray", color: "text-green-600", bg: "bg-green-100" };
      case "connecting":
        return { text: "Connecting to QZ Tray...", color: "text-yellow-600", bg: "bg-yellow-100" };
      case "disconnected":
        return { text: "Disconnected from QZ Tray", color: "text-red-600", bg: "bg-red-100" };
      default:
  ...
  return (
    <div className={`p-3 rounded-lg ${statusInfo.bg} ${statusInfo.color} mb-4`}>
  ...
  const connectToQz = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // Mock successful connection with printers
        const mockPrinters = ["POS-58", "Receipt-Printer", "Thermal-Printer-80mm"];
        setQzPrinters(mockPrinters);
