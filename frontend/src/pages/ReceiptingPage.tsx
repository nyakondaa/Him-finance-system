...
import { Printer, Receipt, Settings, Wifi, WifiOff, RefreshCw, AlertCircle, CheckCircle, Loader2 } from 'lucide-react';
import { getBranches, getRevenueHeads, createTransaction } from '../services/api';
...
// Fixed QZ Tray connection with proper callback pattern and server URLs
    const connectToQz = useCallback(async () => {
        ...
        try {
            ...
            window.qz.security.setCertificatePromise(() => {
                ...
                return fetch(`${API_BASE_URL}/qz/certificate`)
                    .then(response => {
            ...
            showModal(
                `QZ Tray connected successfully! Found ${printers.length} printer(s).${useSimpleMode ? ' (Development mode)' : ''}`,
            ...
            let errorMessage = 'Could not connect to QZ Tray. Please ensure QZ Tray is running and try again.';

            if (err.message && (err.message.includes('already exists') || err.message.includes('already connected'))) {
                ...
                try {
                    ...
                    setSelectedPrinter(printers[0] || '');
                    setQzConnection('connected');
                    showModal('QZ Tray reconnected successfully!', 'Connection Successful', 'success');
                    return;
                } catch (printerError) {
    ...
    const printReceipt = async (receiptData) => {
        ...
        try {
            const selectedBranch = branches.find(b => b.code === receiptData.branchCode);
            const selectedRevenueHead = revenueHeads.find(r => r.code === receiptData.revenueHeadCode);
            ...
            <body>
                ...
                <div class="center">${selectedBranch?.name || 'Unknown Branch'}</div>
                <div class="divider"></div>
                
                <div><strong>Receipt #:</strong> ${receiptData.receiptNumber || 'N/A'}</div>
                <div><strong>Date:</strong> ${new Date(receiptData.transactionDate || new Date()).toLocaleDateString()}</div>
                <div><strong>Time:</strong> ${new Date(receiptData.transactionDate || new Date()).toLocaleTimeString()}</div>
                <div><strong>Payer:</strong> ${receiptData.payerName}</div>
                <div><strong>Revenue Head:</strong> ${selectedRevenueHead?.name || 'Unknown Head'}</div>
                <div><strong>Payment Method:</strong> ${receiptData.paymentMethod}</div>
                
                <div class="divider"></div>
                
                <div class="amount-box">
                ${receiptData.currency} ${(parseFloat(receiptData.amount) || 0).toFixed(2)}
                </div>
        ...
        `;
            ...
            let errorMessage = 'Failed to print receipt. Please check printer connection and try again.';
            ...
            if (err.message && err.message.includes('printer')) {
                ...
                errorMessage = 'Connection error. Please ensure QZ Tray is running and the printer is connected.';
            }
...
// Add this helper function to test QZ Tray without printing
    ...
    const handleGenerateReceipt = async () => {
        if (!isFormComplete) {
            showModal('Please fill in all required fields.', 'Validation Error', 'error');
            return;
        }
        ...
        if (!user) {
            showModal('User information not found. Please login again.', 'Authentication Error', 'error');
            return;
        }
        ...
        try {
            ...
            await printReceipt({
                ...
                transactionDate: createdTransaction.transactionDate || new Date().toISOString()
            });
                            ...
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setReceipt({...receipt, payerName: e.target.value})}
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setReceipt({...receipt, branchCode: e.target.value, revenueHeadCode: ''})}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Revenue Head *</label>
                                    <select
                                    ...
                                    <div className="mt-2 text-sm">
                                        <div className="flex items-center">
                                            <div className={`w-3 h-3 rounded-full mr-2 ${
                                                qzConnection === 'connected' ? 'bg-green-500' :
                                                    qzConnection === 'connecting' ? 'bg-yellow-500' :
                                                        'bg-red-500'
                                            }`}></div>
                                            <span>
                                                QZ Tray: {qzConnection === 'connected' ? 'Connected' :
                                                qzConnection === 'connecting' ? 'Connecting...' :
                                                    'Disconnected'}
                                            </span>
                                        </div>
                                        <button
                                        ...
                                        >
                                            ...
                                            {qzConnection === 'connecting' ? 'Connecting...' : 'Reconnect'}
                                        </button>
                                    </div>
                                </div>
                            ...
                            {isFormComplete ? (
                                <div className="bg-white p-6 border-2 border-dashed border-gray-300 font-mono text-sm rounded-lg max-w-sm mx-auto">
                                    <div className="text-center mb-4">
                                        <h3 className="font-bold text-lg text-gray-800">HIM ADMINISTRATION</h3>
                                        <p className="text-gray-600 text-xs">
                                            {branches.find(b => b.code === receipt.branchCode)?.name || 'Unknown Branch'}
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-1 mb-4 text-gray-700 text-xs">
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <span className="truncate ml-2">
                                                {revenueHeads.find(r => r.code === receipt.revenueHeadCode)?.name || 'Unknown Head'}
                                            </span>
                                        </div>
                                        <div className="flex justify-between">
                                    ...
                                    <div className="text-2xl font-bold text-center mt-6 mb-6 p-3 bg-blue-50 text-blue-800 rounded-md border-2 border-solid border-blue-200">
                                        {receipt.currency} {(parseFloat(receipt.amount) || 0).toFixed(2)}
                                    </div>
                                    ...
                                    <div className="text-center text-xs text-gray-500">
                                        ...
                                        <p className="mt-1">Operator: {user?.username || 'Current User'}</p>
            ...
            <div className="bg-white rounded-lg shadow-xl p-6 max-w-sm w-full mx-4">
                <div className="flex items-center mb-4">
                    {type === 'success' ? (
                        ...
                        <AlertCircle className={`w-6 h-6 ${type === 'error' ? 'text-red-500' : 'text-blue-500'}`}/>
                    )}
