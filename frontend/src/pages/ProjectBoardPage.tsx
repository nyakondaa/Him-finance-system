...
const ProjectBoardPage = ({ showModal }) => {
  ...
  const getCurrencySymbol = useCallback(
    (code) => currencies.find((c) => c.code === code)?.symbol || code,
    [currencies]
  );
  ...
  const deleteProjectMutation = useMutation({
    ...
    onError: (err) =>
      showModal?.(err.message || "Failed to delete project.", "Error"),
  });
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th
                  onClick={() => requestSort("targetAmount")}
                  className={`py-4 px-6 text-left text-sm font-semibold text-gray-700 cursor-pointer`}
                >
                  Target Amount
                </th>
                    ...
                    <tr className="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <td className="py-4 px-6 flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-lg text-xs font-medium flex items-center gap-1 bg-blue-100 text-blue-700 hover:bg-blue-200"
                        >
                          <Edit className="w-3 h-3" /> Edit
                        </button>
                        <button
                          onClick={() => handleDeleteProject(project.id)}
                          className="px-4 py-2 rounded-lg text-xs font-medium flex items-center gap-1 bg-red-100 text-red-700 hover:bg-red-200"
                        >
