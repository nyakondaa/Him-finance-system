...
import {
    TrendingUp, TrendingDown, DollarSign, Users, Building, Calendar,
    Clock, CreditCard, Smartphone, Banknote, PiggyBank, Target,
    Activity, Zap, Award, Eye, BarChart3, PieChart, ArrowUpRight,
    ArrowDownRight, Plus, Minus, AlertTriangle, CheckCircle,
    RefreshCw, Filter, Download, Settings, Bell, Search,
    MapPin, Timer, Wallet, Receipt, Star, Crown, Flame, Square, CheckSquare
} from 'lucide-react';
import {
    LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer,
    PieChart as RechartsPie, Pie, Cell, AreaChart, Area, RadialBarChart, RadialBar
} from 'recharts';
import { getTransactions, getUsers, getBranches, getRevenueHeads, getCurrencies, getPaymentMethods } from '../services/api';
...
const Dashboard = ({ showModal }) => {
    ...
    // Fetch all data
    const { data: transactionsData, isLoading: loadingTransactions, isError: isErrorTransactions, error: transactionsError } = useQuery({
    ...
    const { data: usersData, isLoading: loadingUsers, isError: isErrorUsers, error: usersError } = useQuery({
        queryKey: ['users'],
        queryFn: getUsers,
        onError: (err) => {
    ...
    const { data: branchesData, isLoading: loadingBranches, isError: isErrorBranches, error: branchesError } = useQuery({
        queryKey: ['branches'],
        queryFn: getBranches,
        onError: (err) => {
    ...
    const { data: revenueHeadsData, isLoading: loadingRevenueHeads, isError: isErrorRevenueHeads, error: revenueHeadsError } = useQuery({
    ...
    const { data: currenciesData, isLoading: loadingCurrencies, isError: isErrorCurrencies, error: currenciesError } = useQuery({
        queryKey: ['currencies'],
    ...
    const { data: paymentMethodsData, isLoading: loadingPaymentMethods, isError: isErrorPaymentMethods, error: paymentMethodsError } = useQuery({
    ...
    const users = useMemo(() => {
        if (!usersData) return [];
        return Array.isArray(usersData) ? usersData : usersData.users || usersData.data || [];
    }, [usersData]);

    const branches = useMemo(() => {
        if (!branchesData) return [];
        return Array.isArray(branchesData) ? branchesData : branchesData.branches || branchesData.data || [];
    }, [branchesData]);

    const revenueHeads = useMemo(() => {
        if (!revenueHeadsData) return [];
        return Array.isArray(revenueHeadsData) ? revenueHeadsData : revenueHeadsData.revenueHeads || revenueHeadsData.data || [];
    }, [revenueHeadsData]);

    const currencies = useMemo(() => {
        if (!currenciesData) return [];
        return Array.isArray(currenciesData) ? currenciesData : currenciesData.currencies || currenciesData.data || [];
    }, [currenciesData]);

    const paymentMethods = useMemo(() => {
        if (!paymentMethodsData) return [];
        return Array.isArray(paymentMethodsData) ? paymentMethodsData :
    ...
    const availableCurrencies = useMemo(() => {
        ...
        const currenciesSet = new Set(transactions.map(t => t.currency).filter(Boolean));
    ...
    const getDateRange = (period) => {
        ...
        switch (period) {
            ...
            case 'prevWeek':
                const prevWeekStart = new Date(today.getFullYear(), today.getMonth(), today.getDate() - today.getDay() - 7);
                const weekStart2 = new Date(today.getFullYear(), today.getMonth(), today.getDate() - today.getDay());
                return { start: prevWeekStart, end: new Date(weekStart2.getTime() - 1) };
            case 'month':
    ...
    const { start: prevStart, end: prevEnd } = getDateRange(
        selectedPeriod === 'today' ? 'yesterday' :
            selectedPeriod === 'week' ? 'prevWeek' :
                selectedPeriod === 'month' ? 'prevMonth' : 'today'
    );
    ...
    const metrics = useMemo(() => {
        ...
        const countChange = previousCount === 0 ? (currentCount > 0 ? 100 : 0) : ((currentCount - previousCount) / previousCount) * 100;

        const avgTransaction = currentCount > 0 ? currentRevenue / currentCount : 0;
        const prevAvgTransaction = previousCount > 0 ? previousRevenue / previousCount : 0;
        const avgChange = prevAvgTransaction === 0 ? (avgTransaction > 0 ? 100 : 0) : ((avgTransaction - prevAvgTransaction) / prevAvgTransaction) * 100;

        const uniquePayersSet = new Set(currentTransactions.map(t => t.payerName).filter(Boolean));
    ...
    const paymentMethodsChartData = useMemo(() => {
        ...
        return Object.values(methods)
            .map((method, index) => ({ ...method, color: colors[index % colors.length] }))
    ...
    const topRevenueHeads = useMemo(() => {
        ...
        currentTransactions.forEach(t => {
            ...
            if (revenueHeadCode) {
                revenueHeadTotals[revenueHeadCode] = (revenueHeadTotals[revenueHeadCode] || 0) + amount;
            }
        });
    ...
    const transactionValueDistributionData = useMemo(() => {
        const bins = [
            ...
            { name: `> ${selectedCurrency} 1000`, min: 1000, max: Infinity, count: 0 },
        ];
    ...
    if (loadingTransactions || loadingUsers || loadingBranches || loadingRevenueHeads || loadingCurrencies || loadingPaymentMethods) {
        return (
    ...
    if (isErrorTransactions || isErrorUsers || isErrorBranches || isErrorRevenueHeads || isErrorCurrencies || isErrorPaymentMethods) {
        return (
            <div className="text-center text-red-600 p-8">
                <h2 className="text-xl font-semibold mb-4">Error Loading Dashboard Data</h2>
                <div className="space-y-2">
                    {isErrorTransactions && (
                        <p className="text-sm">Transactions: {transactionsError?.message || 'Unknown error'}</p>
                    )}
                    {isErrorUsers && (
                        <p className="text-sm">Users: {usersError?.message || 'Unknown error'}</p>
                    )}
                    {isErrorBranches && (
                        <p className="text-sm">Branches: {branchesError?.message || 'Unknown error'}</p>
                    )}
                    {isErrorRevenueHeads && (
                        <p className="text-sm">Revenue Heads: {revenueHeadsError?.message || 'Unknown error'}</p>
                    )}
                    {isErrorCurrencies && (
                        <p className="text-sm">Currencies: {currenciesError?.message || 'Unknown error'}</p>
                    )}
                    {isErrorPaymentMethods && (
                        <p className="text-sm">Payment Methods: {paymentMethodsError?.message || 'Unknown error'}</p>
                    )}
                </div>
                <button
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <label className="flex items-center cursor-pointer text-gray-700 text-sm sm:text-base">
                                ...
                                <span className="w-5 h-5 sm:w-6 h-6 border-2 border-gray-400 rounded-md flex items-center justify-center mr-2 transition-all duration-200 ease-in-out flex-shrink-0">
                                    {isDefaultCurrency ? <CheckSquare className="w-4 h-4 sm:w-5 h-5 text-blue-600" /> : <Square className="w-4 h-4 sm:w-5 h-5 text-gray-400" />}
                                </span>
                                Default
                            </label>
            ...
            <div className="max-w-7xl mx-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className={`flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-medium ${getPerformanceColor(metrics.revenueChange)}`}>
                                {getPerformanceIcon(metrics.revenueChange)}
                                <span>{metrics.revenueChange.toFixed(1)}%</span>
                            </div>
                        </div>
                        <div className="space-y-1">
                            <p className="text-sm font-medium text-gray-600">Total Revenue ({selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</p>
                            <p className="text-3xl font-bold text-gray-900">{formatCurrency(metrics.totalRevenue)}</p>
                            <p className="text-xs text-gray-500">
                                {selectedPeriod === 'today' ? 'vs yesterday' : `vs previous ${selectedPeriod}`}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className={`flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-medium ${getPerformanceColor(metrics.transactionChange)}`}>
                                {getPerformanceIcon(metrics.transactionChange)}
                                <span>{metrics.transactionChange.toFixed(1)}%</span>
                            </div>
                        </div>
                        <div className="space-y-1">
                            <p className="text-sm font-medium text-gray-600">Transactions ({selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</p>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className={`flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-medium ${getPerformanceColor(metrics.averageChange)}`}>
                                {getPerformanceIcon(metrics.averageChange)}
                                <span>{metrics.averageChange.toFixed(1)}%</span>
                            </div>
                        </div>
                        <div className="space-y-1">
                            <p className="text-sm font-medium text-gray-600">Avg. Transaction ({selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</p>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300">
                        ...
                        <div className="space-y-1">
                            <p className="text-sm font-medium text-gray-600">Unique Payers ({selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</p>
                            <p className="text-3xl font-bold text-gray-900">{formatNumber(metrics.uniquePayers)}</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-bold text-gray-900">Revenue Timeline ({selectedPeriod === 'today' ? 'Hourly' : 'Daily'})</h3>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-bold text-gray-900">Payment Mix ({selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</h3>
                                <p className="text-sm text-gray-500">Method distribution by value</p>
                            </div>
                            ...
                            {paymentMethodsChartData.slice(0, 4).map((method) => (
                                <div key={method.name} className="flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        <p className="text-sm font-bold text-gray-900">{formatCurrency(method.value)}</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-bold text-gray-900">Top Payers ({selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</h3>
                                <p className="text-sm text-gray-500">Revenue leaders by customer</p>
                            </div>
                        ...
                        <div className="space-y-4">
                            {topPayers.length > 0 ? (
                                topPayers.map((payer, index) => (
                                    <div key={payer.name} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                <p className="font-medium text-gray-900 text-sm">{payer.name}</p>
                                                <p className="text-xs text-gray-500">{payer.count} transactions</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-gray-900">{formatCurrency(payer.total)}</p>
                                            <p className="text-xs text-gray-500">
                                ...
                                <p className="text-gray-500 text-center py-4 text-sm">No top payers for this period.</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        ...
                        <div className="space-y-3">
                            {currentTransactions.slice(-5).reverse().map((transaction) => (
                                <div key={transaction.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                                    <div className="flex items-center space-x-3">
                                        <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center">
                                            <Receipt className="w-4 h-4 text-blue-600" />
                                        </div>
                                        <div>
                                            <p className="font-medium text-gray-900 text-sm">{transaction.payerName || 'Unknown Payer'}</p>
                                            <p className="text-xs text-gray-500">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-xs text-gray-500">{transaction.paymentMethod || 'Unknown Method'}</p>
                                    </div>
                                </div>
                            ))}
                            {currentTransactions.length === 0 && (
                                <p className="text-gray-500 text-center py-4 text-sm">No recent transactions for this period.</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-bold text-gray-900">Currencies & Methods</h3>
                                <p className="text-sm text-gray-500">Supported payment options</p>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div className="p-3 bg-blue-50 rounded-xl">
                                <h4 className="font-medium text-gray-900 mb-2 text-sm">Active Currencies</h4>
                            ...
                            <div className="p-3 bg-purple-50 rounded-xl">
                                <h4 className="font-medium text-gray-900 mb-2 text-sm">Payment Methods</h4>
                                <div className="space-y-2">
                                    {paymentMethods.length > 0 ? (
                                        paymentMethods.slice(0, 5).map(method => (
                                            <div key={method.id || method.name} className="flex items-center justify-between">
                ...
                <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 text-white">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="text-center">
                            ...
                            <p className="text-3xl font-bold">{metrics.revenueChange > 0 ? '+' : ''}{metrics.revenueChange.toFixed(1)}%</p>
                        </div>
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-white/80 text-sm mb-1">Transaction Success Rate</p>
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-white/80 text-sm mb-1">Peak Hour ({selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</p>
                            <p className="text-3xl font-bold">
                                {hourlyData.reduce((max, curr) => curr.revenue > max.revenue ? curr : max, hourlyData[0])?.hourLabel || '--:--'}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-6 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    <h3 className="text-lg font-semibold mb-4 text-gray-700 flex items-center gap-2">
                        <DollarSign className="w-5 h-5 text-gray-600" /> Transaction Value Distribution ({selectedCurrency}, {selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})
                    </h3>
                    <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart
                ...
                <div className="mt-6 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    <h3 className="text-lg font-semibold mb-4 text-gray-700">Payment Methods Distribution ({selectedCurrency}, {selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</h3>
                    <div className="space-y-4">
                        {paymentMethodsChartData.length > 0 ? (
                            paymentMethodsChartData.map(method => {
                                const percentage = metrics.totalRevenue > 0 ? (method.value / metrics.totalRevenue * 100) : 0;
                                return (
                                    <div key={method.name} className="space-y-1.5">
                                        <div className="flex justify-between text-sm text-gray-700">
                                            <span>{method.name}</span>
                                            <span className="font-semibold">{formatCurrency(method.value)} ({percentage.toFixed(1)}%)</span>
                                        </div>
                            ...
                            <p className="text-gray-500 text-center py-4 text-sm">No payment method data for this period.</p>
                        )}
                    </div>
                </div>
                ...
                {/* Branch Performance */}
                <div className="mt-6 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    <h3 className="text-lg font-semibold mb-4 text-gray-700">Branch Performance ({selectedCurrency}, {selectedPeriod === 'today' ? 'Today' : selectedPeriod === 'week' ? 'This Week' : 'This Month'})</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {branchPerformanceData.length > 0 ? (
                            branchPerformanceData.map(branch => (
                                <div key={branch.name} className="p-4 border border-gray-200 rounded-xl bg-gray-50 hover:shadow-md transition duration-200">
                                    <div className="flex items-center gap-2 mb-2">
                                        <Building className="w-4 h-4 text-gray-600" />
                                        <span className="font-semibold text-gray-800 text-sm">{branch.name}</span>
                                    </div>
                                    <p className="text-2xl font-bold text-blue-700">{formatCurrency(branch.total)}</p>
