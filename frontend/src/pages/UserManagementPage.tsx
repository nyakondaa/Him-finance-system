...
import {
    ...
    Mail, Phone, Calendar, Activity, AlertTriangle, RefreshCw
} from 'lucide-react';
import {
...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ showModal }) => {
    ...
    const updateUserMutation = useMutation<{ id: number; data: any }, any, { id: number; data: any }>({
        ...
        onError: (err) => showModal(err.message || 'Failed to update user.', 'Error'),
    });
    ...
    // Check permissions
    const canManageUsers = currentUser?.permissions?.users?.includes('read') || false;
    ...
    if (!canManageUsers) {
        return (
            <div className="bg-white p-8 rounded-xl shadow-lg text-center text-gray-700">
                ...
                <p className="text-gray-500 mt-2">You do not have permission to access User Management.</p>
            </div>
        );
    }
                ...
                <div className="p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <select
                            ...
                            onChange={(e) => editingUser
                                ? setEditingUser({ ...editingUser, branchCode: e.target.value })
                    ...
                    <div className="flex gap-4 mt-6">
                        <button
                            ...
                            disabled={
                                (editingUser ? !editingUser.username || !editingUser.roleId || !editingUser.branchCode :
                                    !newUser.username || !newUser.password || !newUser.roleId || !newUser.branchCode) ||
                                passwordError ||
                ...
                <div className="overflow-x-auto px-8 pb-8">
                    <table className="w-full border-collapse rounded-lg overflow-hidden">
                        ...
                        <tr className="bg-gray-100 text-gray-700 uppercase text-sm leading-normal">
                            <th className="py-3 px-6 text-left border-b border-gray-200">User Info</th>
                            ...
                            <th className="py-3 px-6 text-left border-b border-gray-200">Activity</th>
                        ...
                        {filteredUsers.map(user => {
                            ...
                            const userBranch = branches.find(b => b.code === user.branchCode);
                            return (
                                <tr key={user.id} className="border-b border-gray-200 hover:bg-gray-50 transition duration-150">
                                    ...
                                    <td className="py-4 px-6">
                                        <div className="flex flex-col gap-1">
                                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                                                ...
                                                {user.locked ? 'Locked' : user.isActive ? 'Active' : 'Inactive'}
                                            </span>
                                            {user.attempts > 0 && (
                                    ...
                                    <td className="py-4 px-6">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            {currentUser?.permissions?.users?.includes('update') && (
                                                <button
                                                    onClick={() => {
                                            ...
                                            {currentUser?.permissions?.users?.includes('lock_unlock') && (
                                                <button
                                                    onClick={() => handleToggleUserLock(user.id, user.locked)}
