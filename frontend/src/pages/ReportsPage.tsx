...
import {
    FileText, Download, Calendar, BarChart2, DollarSign, Receipt,
    TrendingUp, Filter, Eye, RefreshCw, PieChart,
    Activity, Clock, Users, Building, CreditCard,
    ArrowUp, ArrowDown, ChevronLeft, ChevronRight,
    Search, Settings, Star, Bookmark, Target,
    Zap, Archive, BookOpen, Layers, AlertCircle,
    Printer, FileSpreadsheet, FileX
} from 'lucide-react';
import {
...
const CHART_COLORS = [
    ...
    '#14B8A6', '#F43F5E', '#6366F1', '#84CC16', '#F97316'
];
...
const PDFReportGenerator = ({
    ...
    const generateAllReports = () => {
        try {
            ...
            generateDailyActivityByCurrencyBranch(doc, transactions, branches, currencies, paymentMethods);
            doc.addPage();
...
const generateDailyActivityByCurrencyBranch = (doc, transactions, branches, currencies) => {
    doc.setFontSize(16);
...
const EnhancedReportsPage = ({ showModal }) => {
    ...
    const processedData = useMemo(() => {
        ...
        const getRevenueHeadName = (code) => revenueHeads.find(h => h.code === code)?.name || code;
        ...
        return {
            ...
            byRevenueHead: groupBy(transactions, t => t.revenueHeadCode, getRevenueHeadName),
    ...
    const totalPages = transactionsData ? Math.ceil(transactionsData.total / ITEMS_PER_PAGE) : 0;
    const startItem = offset + 1;
    const endItem = Math.min(offset + ITEMS_PER_PAGE, transactionsData?.total || 0);
    ...
    const exportToCSV = () => {
        if (!processedData?.transactions?.length) {
            showModal?.('No data to export. Please generate a report first.', 'Warning');
            return;
        }

        const csvContent = [
            ['Date', 'Receipt Number', 'Payer', 'Branch', 'Revenue Head', 'Amount', 'Currency', 'Payment Method', 'Operator'],
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600 mt-1">Comprehensive transaction analytics and reports</p>
                            </div>
                        </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 mb-8 overflow-hidden">
                    ...
                    <div className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Revenue Head</label>
                                <select
                                    value={filters.revenueHeadCode}
                                    onChange={(e) => handleFilterChange('revenueHeadCode', e.target.value)}
                                    className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                >
                                    <option value="">All Revenue Heads</option>
                                    {revenueHeads.map(head => (
                                        <option key={head.code} value={head.code}>{head.name}</option>
                                    ))}
                                </select>
                            </div>
                                ...
                                {transactionsData && (
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                                        <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-2xl font-bold text-blue-700">{formatNumber(transactionsData.total)}</p>
                                                </div>
                                        ...
                                        <div className="bg-green-50 rounded-xl p-4 border border-green-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-green-600 text-sm font-medium">Page Records</p>
                                                    <p className="text-2xl font-bold text-green-700">{formatNumber(transactionsData.transactions?.length || 0)}</p>
                                                </div>
                                        ...
                                        <div className="bg-purple-50 rounded-xl p-4 border border-purple-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-2xl font-bold text-purple-700">{currentPage} of {totalPages}</p>
                                                </div>
                                        ...
                                        <div className="bg-orange-50 rounded-xl p-4 border border-orange-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-orange-600 text-sm font-medium">Showing</p>
                                                    <p className="text-2xl font-bold text-orange-700">{startItem}-{endItem}</p>
                                                </div>
                                ...
                                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                                    <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                                        ...
                                        {transactionsData && (
                                            <span className="text-sm text-gray-500">
                                                Showing {startItem}-{endItem} of {formatNumber(transactionsData.total)} transactions
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    <div className="overflow-x-auto">
                                        <table className="w-full">
                                            ...
                                            {!transactionsError && transactionsData?.transactions?.length > 0 ? (
                                                transactionsData.transactions.map((t) => (
                                                    <tr key={t.id || t.receiptNumber} className="hover:bg-gray-50 transition-colors">
                            ...
                            <div className="space-y-8">
                                {!processedData ? (
                                    <div className="text-center py-12 text-gray-500">
                                        ...
                                        <p>Please generate a report to see charts and visualizations.</p>
                                    </div>
                                ) : (
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                        {/* Revenue Over Time */}
                                        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><TrendingUp className="w-5 h-5 mr-2 text-blue-500"/>Revenue Over Time</h3>
                                            <ResponsiveContainer width="100%" height={300}>
                                                <AreaChart data={processedData.byDate} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                                    <CartesianGrid strokeDasharray="3 3" />
                                                    <XAxis dataKey="date" tickFormatter={(date) => new Date(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} />
                                                    <YAxis tickFormatter={(value) => `$${(value / 1000).toFixed(0)}k`} />
                                                    <Tooltip formatter={(value, name) => [formatCurrency(value), 'Amount']} />
                                                    <Legend />
                                        ...
                                        {/* Revenue by Branch */}
                                        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><Building className="w-5 h-5 mr-2 text-green-500"/>Revenue by Branch</h3>
                                            <ResponsiveContainer width="100%" height={300}>
                                                <BarChart data={processedData.byBranch.slice(0, 10)} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                                    <CartesianGrid strokeDasharray="3 3" />
                                        ...
                                        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><Layers className="w-5 h-5 mr-2 text-purple-500"/>Top Revenue Heads</h3>
                                            <ResponsiveContainer width="100%" height={300}>
                                                <BarChart data={processedData.byRevenueHead.slice(0, 10)}>
                                                    <CartesianGrid strokeDasharray="3 3" />
                                                    <XAxis dataKey="name" tick={{ fontSize: 10 }} angle={-25} textAnchor="end" height={60} />
                                                    <YAxis />
                                                    <Tooltip formatter={(value) => formatCurrency(value)} />
                                                    <Bar dataKey="amount" fill="#8B5CF6">
                                                        {processedData.byRevenueHead.slice(0, 10).map((entry, index) => (
                                        ...
                                        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><CreditCard className="w-5 h-5 mr-2 text-yellow-500"/>Payment Methods</h3>
                                            <ResponsiveContainer width="100%" height={300}>
                                                <RechartsPieChart>
                                                    <Pie data={processedData.byPaymentMethod} dataKey="amount" nameKey="name" cx="50%" cy="50%" outerRadius={100} fill="#F59E0B" label>
                                                        {processedData.byPaymentMethod.map((entry, index) => (
                                                            <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} />
                                                        ))}
                                                    </Pie>
                                                    <Tooltip formatter={(value) => formatCurrency(value)} />
                                    ...
                                    <>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                            <div className="bg-gradient-to-br from-blue-100 to-blue-200 p-6 rounded-2xl shadow-md border border-blue-200">
                                                ...
                                                <p className="text-3xl font-bold text-blue-900">{formatCurrency(processedData.overview.totalAmount)}</p>
                                            </div>
                                            <div className="bg-gradient-to-br from-green-100 to-green-200 p-6 rounded-2xl shadow-md border border-green-200">
                                                ...
                                                <p className="text-3xl font-bold text-green-900">{formatNumber(processedData.overview.totalTransactions)}</p>
                                            </div>
                                            <div className="bg-gradient-to-br from-purple-100 to-purple-200 p-6 rounded-2xl shadow-md border border-purple-200">
                                                ...
                                                <p className="text-3xl font-bold text-purple-900">{formatCurrency(processedData.overview.averageTransaction)}</p>
                                            </div>
                                            <div className="bg-gradient-to-br from-yellow-100 to-yellow-200 p-6 rounded-2xl shadow-md border border-yellow-200">
                                                ...
                                                <p className="text-sm font-medium text-yellow-800">Unique Payers</p>
                                                <p className="text-3xl font-bold text-yellow-900">{formatNumber(processedData.overview.uniquePayers)}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                                            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><Star className="w-5 h-5 mr-2 text-yellow-500" />Top 5 Branches by Revenue</h3>
                                                <ul className="space-y-3">
                                                    {processedData.byBranch.slice(0, 5).map((item, index) => (
                                            ...
                                            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><Bookmark className="w-5 h-5 mr-2 text-blue-500" />Top 5 Revenue Heads</h3>
                                                <ul className="space-y-3">
                                                    {processedData.byRevenueHead.slice(0, 5).map((item, index) => (
                                                        <li key={index} className="flex justify-between items-center text-sm">
                                                            <span className="text-gray-700">{item.name}</span>
                                                            <span className="font-bold text-gray-900">{formatCurrency(item.amount)}</span>
                                                        </li>
                                            ...
                                            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><PieChart className="w-5 h-5 mr-2 text-green-500" />Payment Method Usage</h3>
                                                <ul className="space-y-3">
                                                    {processedData.byPaymentMethod.map((item, index) => (
                                                        <li key={index} className="flex justify-between items-center text-sm">
                                                            <span className="text-gray-700">{item.name} ({item.count} times)</span>
                                                            <span className="font-bold text-gray-900">{formatCurrency(item.amount)}</span>
                                                        </li>
                                                    ))}
                                                </ul>
