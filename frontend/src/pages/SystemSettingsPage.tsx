...
import { getBranches, addBranch, updateBranch, deleteBranch } from '../services/api';
import LoadingSpinner from '../components/LoadingSpinner';
import useAuth from '../hooks/useAuth';

const SystemSettingsPage = ({ showModal }) => {
    ...
    const handleAddBranch = () => {
        if (!newBranchCode || !newBranchName) {
            showModal('Branch Code and Name cannot be empty.', 'Validation Error');
            return;
        }
    ...
    const handleUpdateBranch = () => {
        if (!editingBranch.name) {
            showModal('Branch Name cannot be empty.', 'Validation Error');
            return;
        }
        updateBranchMutation.mutate({ code: editingBranch.code, data: { name: editingBranch.name } });
    };

    const handleDeleteBranch = (code, name) => {
        showModal(
    ...
    if (!(currentUser?.role === 'supervisor' || currentUser?.role === 'admin')) {
        return (
            <div className="bg-white p-8 rounded-xl shadow-lg text-center text-gray-700">
                <p className="text-lg font-semibold">You do not have permission to access System Settings.</p>
        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg mb-8 border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <input
                        type="text"
                        placeholder={editingBranch ? "Branch Name" : "New Branch Name (e.g., Gweru Branch)"}
                ...
                <div className="flex gap-4 mt-6">
                    <button
                        ...
                        disabled={(editingBranch ? !editingBranch.name : (!newBranchCode || !newBranchName)) || addBranchMutation.isLoading || updateBranchMutation.isLoading}
                    >
                        {addBranchMutation.isLoading || updateBranchMutation.isLoading ? 'Saving...' : editingBranch ? <><Save className="w-5 h-5" /> Update Branch</> : <><Plus className="w-5 h-5" /> Add Branch</>}
                    </button>
                    {editingBranch && (
                        <button
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-200">
                <h3 className="text-xl font-semibold mb-6 text-gray-700">Existing Branches</h3>
                <div className="overflow-x-auto">
                    <table className="w-full border-collapse rounded-lg overflow-hidden">
                        <thead>
                        <tr className="bg-gray-100 text-gray-700 uppercase text-sm leading-normal">
                            <th className="py-3 px-6 text-left border-b border-gray-200">Code</th>
                        ...
                        {branches.map(branch => (
                            <tr key={branch.code} className="border-b border-gray-200 hover:bg-gray-50 transition duration-150">
                                <td className="py-3 px-6 text-left font-mono">{branch.code}</td>
                                ...
                                <td className="py-3 px-6 text-left flex items-center gap-2">
                                    ...
                                    <button
                                        onClick={() => handleDeleteBranch(branch.code, branch.name)}
                                    ...
                                    >
                                        {deleteBranchMutation.isLoading ? 'Deleting...' : <><Trash2 className="w-3 h-3" /> Delete</>}
                                    </button>
                                </td>
                            </tr>
                        ))}
